...
Audiobooks.getFreshSession = async function () {
  ...
  // getSession() se contente de relire le cache local, sans vérifier si le
  // jeton est encore accepté par le serveur. On fait donc une vraie
  // vérification serveur avec getUser() avant de faire confiance à ce jeton.
  const { data: userCheck, error: userErr } = await sb.auth.getUser();

  if (userErr || !userCheck || !userCheck.user) {
    const { data: refreshed, error: refreshErr } = await sb.auth.refreshSession();
    if (refreshErr || !refreshed || !refreshed.session) {
      // Le jeton est irrécupérable : on repart d'une connexion complète et propre.
      await sb.auth.signOut();
      const ok = await Ebooks.login();
      if (!ok) return null;
