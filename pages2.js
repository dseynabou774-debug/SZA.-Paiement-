...
Pages.saleNew = function (presetClientId) {
  if (!Pages.saleDraft) {
    Pages.saleDraft = { clientId: presetClientId || '', items: [], discount: 0, paid: 0, method: 'Espèces', date: new Date().toISOString().slice(0, 16) };
  }
  ...
  return `
    ...
    <div class="card mb-8">
      ...
      <div class="total-box" id="sale-totals">
        ${Pages.saleTotalsHtml(subtotal, discount, total, d.paid, remaining)}
      </div>
    </div>
...
Pages.saleTotalsHtml = function (subtotal, discount, total, paid, remaining) {
  return `
...
Pages.refreshSaleTotals = function () {
  ...
  const total = Math.max(0, subtotal - discount);
  const remaining = Math.max(0, total - Number(d.paid || 0));
  document.getElementById('sale-totals').innerHTML = Pages.saleTotalsHtml(subtotal, discount, total, d.paid, remaining);
};
...
Pages.receipt = function (id) {
  ...
  const ebookLinks = s.items.filter(it => it.isEbook).map(it => {
    const book = DB.getBook(it.bookId);
    return (book && book.downloadLink) ? `<div class="stock-row"><span class="text-muted">📎 ${Utils.escapeHtml(it.title)}</span><a href="${Utils.escapeHtml(book.downloadLink)}" target="_blank" style="color:var(--emerald-700);font-weight:700;font-size:.8rem">Lien de téléchargement</a></div>` : '';
  }).join('');
...
Pages.receiptText = function (id) {
  ...
  const subtotal = s.subtotal != null ? s.subtotal : s.items.reduce((sum, it) => sum + it.price * it.qty, 0);
  const discount = Number(s.discount || 0);
  const ebookLinkLines = s.items.filter(it => it.isEbook).map(it => {
    const book = DB.getBook(it.bookId);
    return (book && book.downloadLink) ? `📎 ${it.title} : ${book.downloadLink}` : null;
  }).filter(Boolean);
  const lines = [
    `🧾 ${set.academyName}`,
...
Pages.reports = function () {
  ...
  const cards = periods.map(p => {
    ...
    const discounts = sales.reduce((s, x) => s + Number(x.discount || 0), 0);
    return `<div class="stat-card"><span class="ic">📅</span><div class="label">${p.label}</div><div class="value">${Utils.money(revenue)}</div><div class="text-muted" style="font-size:.7rem;margin-top:4px">${sales.length} vente(s)${discounts ? ' · -' + Utils.money(discounts) + ' réduc.' : ''}</div></div>`;
  }).join('');
