...
Pages.openAudioDetails = function (id) {
  ...
  Utils.openModal(`
    ...
    <div class="stock-row"><span class="text-muted">Prix</span><strong>${Utils.money(a.price)}</strong></div>
    <div class="stock-row"><span class="text-muted">Langue</span><strong>${Utils.escapeHtml(a.language || '—')}</strong></div>
    ${a.duration_seconds ? `<div class="stock-row"><span class="text-muted">Durée</span><strong>${Math.round(a.duration_seconds/60)} min</strong></div>` : ''}
    ${a.description ? `<div class="mt-8 text-muted" style="font-size:.85rem">${Utils.escapeHtml(a.description)}</div>` : ''}
    <div class="tag-row mt-16">
      <button class="btn btn-outline" onclick="Utils.closeModal()">Fermer</button>
    </div>
  `);
};
